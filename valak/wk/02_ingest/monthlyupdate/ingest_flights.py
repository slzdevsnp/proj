#!/usr/bin/env python3  ##szi changed from python to python3 to force it (python3 is availabe in gcp console vm)
...
def download(YEAR, MONTH, destdir):
   ...
   # szi in  PARAMS  string vars YEAR and MONTH are injected
   ...
   with open(filename, "wb") as fp:         # szi check wb modifier
...
def zip_to_csv(filename, destdir):
   ...
   cwd = os.getcwd()                             #curdir
   os.chdir(destdir)
   zip_ref.extractall()                          #actuall unzipping
   os.chdir(cwd)                                 #get back to curdir
   csvfile = os.path.join(destdir, zip_ref.namelist()[0])
   zip_ref.close()
   logging.info("Extracted {}".format(csvfile))
   return csvfile                               #return a csv file name
...
def verify_ingest(csvfile):
   ...
   with open(csvfile, 'r') as csvfp:
      firstline = csvfp.readline().strip()                      #rm leading, trailing whitespaces
      if (firstline != expected_header):
         os.remove(csvfile)
         msg = 'Got header={}, but expected={}'.format(
                             firstline, expected_header)
         logging.error(msg)
         raise UnexpectedFormat(msg)                            #this exception is defined in local class

      if next(csvfp, None) == None:                             #check for a next (2nd) line in a file
         os.remove(csvfile)
   ...
   with open(csvfile, 'r') as infp:
     with open(outfile, 'w') as outfp:
        for line in infp:
           ...
           outline = remove_quote(outline)                  #call a local func
           outfp.write(outline)
           outfp.write('\n')
...
def upload(csvfile, bucketname, blobname):
   ...
   blob = Blob(blobname, bucket)            #blobname is a file path inside a bucket. it is stored as a blob
...
def ingest(year, month, bucket):
   ...
   try:
      zipfile = download(year, month, tempdir)                      #call a local func
      bts_csv = zip_to_csv(zipfile, tempdir)                        #call a local func
      csvfile = remove_quotes_comma(bts_csv, year, month)           #call a local func
      verify_ingest(csvfile)                                        #call a local func
      gcsloc = 'flights/raw/{}'.format(os.path.basename(csvfile))
      return upload(csvfile, bucket, gcsloc)                        #call a local func
   finally:
...
if __name__ == '__main__':
   ...
   try:
      ...
      gcsfile = ingest(year, month, args.bucket)                    #call a local top processing funct
