...
def tz_correct(line, airport_timezones):
   ...
   if fields[0] != 'FL_DATE' and len(fields) == 27:
        ...
        for f in [13, 14, 17]: #crsdeptime, deptime, wheelsoff
            fields[f] = as_utc(fields[0], fields[f], dep_timezone)  #local func call
        for f in [18, 20, 21]: #wheelson, crsarrtime, arrtime
            fields[f] = as_utc(fields[0], fields[f], arr_timezone)  #local func call
...
if __name__ == '__main__':
   with beam.Pipeline('DirectRunner') as pipeline:
        ...
        airports = (pipeline
         ...
         | 'airports:tz' >> beam.Map(lambda fields: (fields[0], addtimezone(fields[21], fields[26])))  #local func call
        ) 
        # airports is ingested here in tzcorr step
        flights = (pipeline
         | 'flights:read' >> beam.io.ReadFromText('201501_part.csv')
         | 'flights:tzcorr' >> beam.FlatMap(tz_correct, beam.pvalue.AsDict(airports))
        )

        flights | beam.io.textio.WriteToText('all_flights')   #output
